...
- `pplx fetch --prompt` removes `user:password@` from the URL before sending it to Perplexity and says so on stderr, so pages behind URL credentials cannot be reached that way. Plain `fetch` sends those credentials as HTTP basic auth.
