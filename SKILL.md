...
- **Read the grounding check.** Every `ask` checks whether the answer's figures (normalized, so `$1.2M` = `1,200,000`) and multi-word names appear in any source's title or snippet. With `-j`: `grounded` is `true`, `false`, or `null` (nothing checkable); `ungrounded_terms` lists the figures/names found in no source; `grounding_reasons` says why (`every cited URL is a site root`, `no sources`, or `N of M figures/names appear ...` when at most 1 in 5 do). An ungrounded answer also gets a `grounded: no (...)` line on stdout and a `warning:` line on stderr; the exit code does not change. `grounded: true` means the terms occur in snippets, not that the answer is right; a figure outside the ~200-character snippet counts as unsupported. `--no-grounded-check` skips it.
